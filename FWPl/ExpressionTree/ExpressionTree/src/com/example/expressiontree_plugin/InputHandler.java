...
public abstract class InputHandler {
    ...
    protected com.example.expressiontree_framework.UserCommand lastValidCommand ;
	
    /** Constructor. */
    InputHandler() {
        treeOps = new TreeOps();
        userCommandFactory = new UserCommandFactory(treeOps);
        com.example.expressiontree_framework.UserCommand f = new com.example.expressiontree_framework.NullCommand(treeOps);
        lastValidCommand = f;
    }
    ...
    public void handleInput() throws Exception {
        ...
        if (input.equals(""))
            ...
            com.example.expressiontree_framework.UserCommand command = makeUserCommand(input);
    ...
    String retrieveInput() {
        return com.example.expressiontree_framework.Platform.instance().retrieveInput(verboseField);
    }
     ...
     * on the user input.
     */
    protected abstract com.example.expressiontree_framework.UserCommand makeUserCommand(String userInput);

    /** This hook method executes a command. */
    void executeCommand(com.example.expressiontree_framework.UserCommand command) throws Exception {
        command.execute();
    }
}