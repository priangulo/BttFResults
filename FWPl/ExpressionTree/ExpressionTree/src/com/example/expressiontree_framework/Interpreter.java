...
public class Interpreter {
    ...
    public class SymbolTable {
        ...
        public void print() {
            for (Iterator<Entry<String, Integer>> it =
                ...
                com.example.expressiontree_framework.Platform.instance().outputLine((x.getKey()
                                                + " = "
                                                + x.getValue()));
            }
        }
     ...
     */
     public void optimizeParseTree(Stack<Symbol> parseTree) {
    }
     ...
     */
     public com.example.expressiontree_framework.ExpressionTree buildExpressionTree(Stack<Symbol> parseTree) {
    ...
    /** Parse next terminal expression. */
     public Stack<Symbol> parseNextSymbol
    ...
    /** Inserts a @a Number into the parse tree. */
     public Stack<Symbol> insertNumberOrVariable(String input,
                                                 int startIndex,
                                                 Stack<Symbol> parseTree,
                                                 boolean isVariable)  {
        /** 
        ...
        if (isVariable)
            /** Lookup the value in the symbolTable. */
                                                           ...
                                                           number =
                new Number(symbolTable.get(input.substring(startIndex, startIndex 
                                                           + endIndex)));
        else
            
                                           number =
                new Number(input.substring(startIndex, startIndex + endIndex));
    ...
    /** Inserts a @a Symbol into the parse tree. */
     public Stack<Symbol> insertSymbolByPrecedence
        (Symbol symbol,
         Stack<Symbol> parseTree) {
        if(!parseTree.empty()) {
            /** 
     ...
     public Stack<Symbol> handleParentheses
        (String inputExpression,
         int index,
         boolean handled,
     ...
     */
    public abstract class Operator extends Symbol { public
        /** Ctor */
     ...
     *        non-terminal expressions.
     */
    public abstract class UnaryOperator extends Symbol { public
        /** Ctor */
