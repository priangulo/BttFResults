 ...
 *        pattern.
 */
public class OutputFilterDecorator extends example.imagetaskgang_framework.FilterDecorator {
    /**
     ...
     * template method.
     */
    public OutputFilterDecorator(example.imagetaskgang_framework.Filter filter) {
    	super(filter);
    }
    ...
    protected ImageEntity decorate(ImageEntity imageEntity) {
        ...
        try{
        	FileOutputStream outputFile = new FileOutputStream(newImage);
                     ...
                     PlatformStrategy.instance().storeImage(imageEntity.getImage(), outputFile);
            } catch (Exception e) {
            e.printStackTrace();
            return null;
        }
