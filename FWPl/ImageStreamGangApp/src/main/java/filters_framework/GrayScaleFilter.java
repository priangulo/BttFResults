 ...
 */
public class GrayScaleFilter extends filters_framework.Filter {
    /**
    ...
    protected utils_framework.Image applyFilter(utils_framework.Image image) {
        ...
        for (int i = 0; i < height; ++i) {
            for (int j = 0; j < width; ++j) {
                ...
                if (
                                            hasTransparent 
                    && (grayScaleImage.getRGB(j, i) >> 24) == 0x00) 
                    continue;
                
                // Convert the pixel to grayscale.
                Color c = 
                                                        new Color(grayScaleImage.getRGB(j, i));
                int grayConversion =
                    (int) (c.getRed() * 0.299)
         ...
         return new utils_framework.Image(image.getSourceURL(),
                         grayScaleImage);
    }
}