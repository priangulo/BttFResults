...
import filters_framework.GrayScaleFilter;
import filters_framework.NullFilter;
...
public class ImageStreamGangTest {
     ...
     */
    private final static filters_framework.Filter[] mFilters = {
        new filters_framework.NullFilter(),
        new filters_framework.GrayScaleFilter()
    };
    ...
    public static void main(String[] args) {
        ...
        utils_framework.Options.instance().parseArgs(args);
    ...
    private static void runTests() {
        ...
        for (TestsToRun test : TestsToRun.values()) {
            ...
            Iterator<List<URL>> urlIterator = 
                utils_framework.Options.instance().getUrlIterator();
            ...
            streams_framework.ImageStreamGang streamGang =
                
                                    
                                    makeImageStreamGang(mFilters, urlIterator, test);
     ...
     */
    private static streams_framework.ImageStreamGang makeImageStreamGang(filters_framework.Filter[] filters,
                                                       Iterator<List<URL>> urlIterator,
                                                       TestsToRun choice) {
        switch (choice) {
    ...
    private static void deleteFilteredImages() {
        ...
        for (filters_framework.Filter filter : mFilters)
            deletedFiles += deleteSubFolders(new File(utils_framework.Options.instance().getDirectoryPath(),
        ...
        for(int treeIndex = 0; treeIndex < numberOfRuns; treeIndex++){
        	for(Entry<String, List<Long>> entry : resultsMap.entrySet()){
                ...
                // whose value is test that ran.
                        ...
                        map.put(entry.getValue().get(treeIndex), entry.getKey());
        	}
        }
//        
