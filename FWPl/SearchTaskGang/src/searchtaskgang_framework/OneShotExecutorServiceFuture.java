 ...
 *        submitted.
 */
public class OneShotExecutorServiceFuture extends searchtaskgang_framework.SearchTaskGangCommon {
    /**
     * A List of Futures that contain SearchResults.
     */
    protected List<Future<searchtaskgang_framework.SearchResults>> mResultFutures;
    ...
    protected void initiateTaskGang(int inputSize) {
        ...
        // SearchResults.
        mResultFutures = 
            new ArrayList<Future<searchtaskgang_framework.SearchResults>> 
    ...
    protected boolean processInput(final String inputData) {
        ...
        for (final String word : mWordsToFind) {
            ...
            // the results.
            final Future<searchtaskgang_framework.SearchResults> resultFuture =
                
                                                 executorService.submit(new Callable<searchtaskgang_framework.SearchResults>() {
                        @Override
                        // call() runs in a background task.
                        public searchtaskgang_framework.SearchResults call() throws Exception {
     ...
     */
    protected void processFutureResults(List<Future<searchtaskgang_framework.SearchResults>> resultFutures) {

        // Iterate through the List of Futures and print the search
        // results.
        for (Future<searchtaskgang_framework.SearchResults> resultFuture : resultFutures) {
            try {
